...
Docker images to run a Terraria Server. Images with [TShock Server](https://github.com/Pryaxis/TShock) or [Vanilla Server](https://terraria.gamepedia.com/Server) are available. The vanilla image supports an interactive console via `docker attach` and graceful shutdown.
...
### Interactive console (vanilla)
The vanilla image forwards the container's stdin to the server console, so you can attach to a running container and type commands directly:
```
sudo docker attach terraria
```
Console commands like `save`, `say <message>`, `kick <player>` and `exit` are handled immediately. Detach with `Ctrl-p Ctrl-q` and the server keeps running.

When the container is stopped (`docker stop` or `docker compose down`), the entrypoint sends `exit` to the server first so the world is saved before shutdown.
